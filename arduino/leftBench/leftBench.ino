...
const unsigned int inPort = 8011;
...
void setModule(int column, int row, int red, int green, int blue) {
  switch (row) {
    ...
    case 1: {
        for (int i = 0; i < PIXELS_PER_MODULE; i++) {
          strip.setPixelColor((UPPER_ROW_MODULES+column) * PIXELS_PER_MODULE + i, strip.Color(red, green, blue));
        }
        break;
      }
  }
...
void routeAll(OSCMessage &msg, int addrOffset) {
  ...
  if (msg.isBlob(0)) {
    ...
    int row = 0;
    setModule(0, row, blob[0], blob[1], blob[2]);
    row = 1;
